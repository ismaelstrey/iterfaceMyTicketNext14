...
import { colorPrioridade, filterEmpresa, mapPrioridadeEnumToValues } from "@/helper/helper";
...
export const NovoTicket = () => {
    ...
    const RenderOptionsCliente = () => empresa.map(({ id, nome }, key): ReactNode => {

        const option = (
            <option key={key} value={Number(id)}>
                {nome}
            </option>
        );
    ...
    const RenderOptions = () => typePrioridade.map((item, key) => {
        const option = (
                ...
                <div className="flex bg-sub-menu p-20 rounded-lg border-2 border-solid border-white relative flex-col gap-4 w-40vw]">
                    <form onSubmit={(e) => handleForm(e)} className="flex flex-col">
                        ...
                        <select
                            name="cliente"
                            id="cliente"
                            value={empresaId ? filterEmpresa(empresa, empresaId) : 'Selecione uma opção'}
                            defaultValue={empresaId ? filterEmpresa(empresa, empresaId) : 'Selecione uma opção'}
                            //@ts-ignore
