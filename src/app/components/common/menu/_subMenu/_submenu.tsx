    ...
    return (
        <div className='bg-menu'>
            <div className='flex w-full h-screen bg-sub-menu border-solid border-r-2 rounded-tl-3xl flex-col'>
                <div className={`flex w-60 h-20  content-center items-center justify-around transition-all duration-500 ${navigate ? '-ml-60' : ' border-solid border-b-2'}`}>
                    <span className='flex text-teal-400'> <FaStarHalfAlt fill="var(--main)" size="2em" /></span>
                    <span className={navigate ? 'text-[var(--sub-menu)]' : 'text-white'}>Filtros Salvos</span>
                    <span onClick={() => { handleToggle && handleToggle() }} title={navigate ? 'Mostrar filtros' : 'Esconder Filtros'} className={`-mr-9 rounded-full z-20 bg-sub-menu p-0 cursor-pointer ${navigate && 'rotate-180'}`}> <IoChevronBackCircleOutline size="2em" fill="black" color="white" /></span>
                </div>
